# FM 5151 — Options Pricing Final Project
...
| Notebook | Content |
|---|---|
| `part2.ipynb` | Black-Scholes pricing, Greeks, put-call parity checks |
| `part3.ipynb` | GBM path simulation, LSM American option pricing |
| `part4.ipynb` | GARCH(1,1) fitting on real stock return data |
| `part5.ipynb` | Variance reduction: antithetic vs. importance sampling comparison |

---

## Notes

- `lsm_price` validates against the Longstaff-Schwartz (2001) paper example at `K=1.10, r=6%, T=3` (8-path toy case), which prices the American put at `$0.1144`.
- GARCH estimation scales returns to percentages internally for better numerical convergence.
- The `antithetic` flag in both `gbm.py` and `lsm.py` requires an even number of paths, since antithetic variates are generated by concatenating `Z` and `-Z`.
